...
if (!$bootstrap->isReady()) {
    ...
    $context = array(
        ...
        'site_description' => isset($_CONF['site_slogan']) ? $_CONF['site_slogan'] : '',
        'page_title' => '',
        'page_description' => '',
        'meta_keywords' => '',
        'content' => ''
    ...
    $analysis = array(
        'mode' => isset($_VIDEOS_CONF['analysis_mode']) ? $_VIDEOS_CONF['analysis_mode'] : 'mixed',
        'maximum' => isset($_VIDEOS_CONF['max_keywords']) ? $_VIDEOS_CONF['max_keywords'] : 8,
        'title_weight' => isset($_VIDEOS_CONF['title_weight']) ? $_VIDEOS_CONF['title_weight'] : 5,
        'meta_weight' => isset($_VIDEOS_CONF['meta_weight']) ? $_VIDEOS_CONF['meta_weight'] : 4,
        'content_weight' => isset($_VIDEOS_CONF['content_weight']) ? $_VIDEOS_CONF['content_weight'] : 1,
        'manual_keywords' => isset($_VIDEOS_CONF['manual_keywords']) ? $_VIDEOS_CONF['manual_keywords'] : '',
        'required_keywords' => isset($_VIDEOS_CONF['required_keywords']) ? $_VIDEOS_CONF['required_keywords'] : '',
        'excluded_keywords' => isset($_VIDEOS_CONF['excluded_keywords']) ? $_VIDEOS_CONF['excluded_keywords'] : ''
    );
    ...
    if ($query === '') {
        ...
        if ($result === false) {
            ...
            $catalogueContextKey = isset($result['cache_key']) && preg_match('/^[a-f0-9]{64}$/', $result['cache_key'])
                ? $result['cache_key'] : '';
            ...
            if (!empty($_VIDEOS_CONF['discovery_enabled'])) {
                $reservoir = new Videos_DiscoveryReservoir($bootstrap->getStore(), $cache);
                ...
                if ($reservoir->isDue($_VIDEOS_CONF)) {
                    $reservoir->refresh(
                        ...
                        $_VIDEOS_CONF,
                        videos_create_public_youtube_service($bootstrap, $_VIDEOS_CONF),
                        false
                    );
                }
            ...
            $selector = new Videos_CatalogueSelector($ratingStats, $privacy, $ranking);
            ...
            if (count($priorityIds) > 0) {
                $existingPriority = isset($selectionConfiguration['priority_channels'])
            ...
            $selection = $selector->select(
                ...
                !empty($_VIDEOS_CONF['account_history_enabled']) && isset($_USER['uid'])
                    ? (int) $_USER['uid'] : 1
            );
            if (!empty($_VIDEOS_CONF['permanent_pool_enabled'])) {
                $pool = new Videos_PermanentPool($bootstrap->getStore(), $cache);
                $poolVideos = $pool->videos($ranking->getGlobal(500), $_VIDEOS_CONF);
                ...
                $selection = $pool->mergeSelections($selection, $poolSelection, $_VIDEOS_CONF);
            }
            ...
            $videos = array_slice($allVideos, ($page - 1) * $perPage, $perPage, true);
            if ($totalVideos === 0) {
...
$faqItems = !empty($_VIDEOS_CONF['faq_catalogue_enabled']) && $page === 1 && count($videos) > 0
...
if (count($videos) > 0) {
    ...
    foreach ($videos as $videoId => $video) {
        ...
        $metadata = isset($videoMetadata[$videoId]) ? $videoMetadata[$videoId] : array();
        $thumbnail = isset($snippet['thumbnails']['medium']['url'])
            ? $snippet['thumbnails']['medium']['url'] : '';
        $url = $_CONF['site_url'] . '/videos/watch.php?v=' . rawurlencode($videoId);
        ...
        if (strpos($thumbnail, 'https://') === 0) {
            $html .= '<img loading="lazy" src="'
                ...
                . '" alt="'
                . htmlspecialchars(VIDEOS_thumbnailAlt($title, $channelTitle), ENT_QUOTES, 'UTF-8')
                . '">';
        }
        ...
        if ($duration > 0) {
            $html .= '<p class="videos-card-meta">'
                . htmlspecialchars($LANG_VIDEOS['video_duration'], ENT_QUOTES, 'UTF-8')
        ...
        if (!empty($metadata['rating_count'])) {
            $html .= '<p class="videos-card-meta">'
                ...
                . ' : ' . number_format((float) $metadata['rating_average'], 2, ',', ' ')
                . '/5 (' . (int) $metadata['rating_count'] . ')</p>';
        }
        if (!empty($metadata['viewed'])) {
            $html .= '<span class="videos-card-badge">'
                . htmlspecialchars($LANG_VIDEOS['already_watched'], ENT_QUOTES, 'UTF-8') . '</span>';
        }
        if (!empty($metadata['permanent_pool'])) {
            $html .= '<span class="videos-card-badge videos-pool-badge">'
                . htmlspecialchars($LANG_VIDEOS['permanent_pool_badge'], ENT_QUOTES, 'UTF-8') . '</span>';
        }
        $html .= '</div></article>';
    }
    $html .= '</div>';
...
$catalogueHeader = $seo->catalogue($publicTitle, $page, count($videos) > 0);
...
function videos_catalogue_pagination($page, $pageCount, $baseUrl, $language)
{
    ...
    if ($page > 1) {
        $html .= '<a rel="prev" href="'
            ...
            . htmlspecialchars($language['previous_page'], ENT_QUOTES, 'UTF-8') . '</a>';
    }
    $html .= '<span>'
        . htmlspecialchars(sprintf($language['catalogue_page'], $page, $pageCount), ENT_QUOTES, 'UTF-8')
        . '</span>';
    if ($page < $pageCount) {
        $html .= '<a rel="next" href="'
            . htmlspecialchars($baseUrl . '?page=' . ($page + 1), ENT_QUOTES, 'UTF-8') . '">'
            . htmlspecialchars($language['next_page'], ENT_QUOTES, 'UTF-8') . '</a>';
    }
    return $html . '</nav>';
}
