...
if (empty($_VIDEOS_CONF['enabled']) || empty($_VIDEOS_CONF['public_rankings_enabled'])) {
    echo COM_createHTMLDocument(
        COM_showMessageText($LANG_VIDEOS['public_rankings_disabled'], '', true),
        array(
            'pagetitle' => $LANG_VIDEOS['rankings'],
            'headercode' => $seo->privatePage($_CONF['site_url'] . '/videos/rankings.php')
        )
    );
    exit;
}
...
$videosEnabled = !empty($_VIDEOS_CONF['public_video_ranking_enabled']);
$channelsEnabled = !empty($_VIDEOS_CONF['public_channel_ranking_enabled']);
...
if ($bootstrap->isReady()) {
    ...
    $blockedVideos = videos_rankings_csv_set(isset($_VIDEOS_CONF['blocked_videos']) ? $_VIDEOS_CONF['blocked_videos'] : '');
    $blockedChannels = videos_rankings_csv_set(isset($_VIDEOS_CONF['blocked_channels']) ? $_VIDEOS_CONF['blocked_channels'] : '');
...
$rankingHasContent = ($activeTab === 'videos' && count($videoItems) > 0)
    || ($activeTab === 'channels' && count($channelItems) > 0);
$faqItems = !empty($_VIDEOS_CONF['faq_rankings_enabled']) && $rankingHasContent
    ? $faqService->rankings($activeTab) : array();
$html = '<div class="videos-page videos-rankings-page">'
    . VIDEOS_renderNavigation('rankings')
    . '<h1>' . htmlspecialchars($LANG_VIDEOS['public_rankings_title'], ENT_QUOTES, 'UTF-8') . '</h1>'
    . '<p class="videos-rankings-intro">' . htmlspecialchars($LANG_VIDEOS['public_rankings_intro'], ENT_QUOTES, 'UTF-8') . '</p>';

if (!$bootstrap->isReady()) {
    $html .= '<p>' . htmlspecialchars($LANG_VIDEOS['public_rankings_unavailable'], ENT_QUOTES, 'UTF-8') . '</p>';
} else {
    if ($videosEnabled || $channelsEnabled) {
        $html .= '<nav class="videos-ranking-tabs" aria-label="'
            . htmlspecialchars($LANG_VIDEOS['ranking_tabs_navigation'], ENT_QUOTES, 'UTF-8') . '"><ul>';
        if ($videosEnabled) {
            $html .= videos_rankings_tab('videos', $activeTab, $LANG_VIDEOS['top_videos'], $_CONF['site_url']);
        }
        if ($channelsEnabled) {
            $html .= videos_rankings_tab('channels', $activeTab, $LANG_VIDEOS['top_channels'], $_CONF['site_url']);
        }
        $html .= '</ul></nav>';
    }
    if ($activeTab === 'videos' && $videosEnabled) {
        $html .= videos_rankings_render_videos($videoItems, $cache, $_CONF, $LANG_VIDEOS);
    }
    if ($activeTab === 'channels' && $channelsEnabled) {
        $html .= videos_rankings_render_channels($channelItems, $LANG_VIDEOS);
    }
    if (!$videosEnabled && !$channelsEnabled) {
        $html .= '<p>' . htmlspecialchars($LANG_VIDEOS['public_rankings_empty'], ENT_QUOTES, 'UTF-8') . '</p>';
    }
}
if (count($faqItems) > 0) {
...
echo COM_createHTMLDocument(
    $html,
    array(
        'pagetitle' => $LANG_VIDEOS['public_rankings_title'] . ' - ' . $publicTitle,
...
{
    $url = $siteUrl . '/videos/rankings.php?tab=' . rawurlencode($tab);
    $html = '<li><a href="' . htmlspecialchars($url, ENT_QUOTES, 'UTF-8') . '"';
    if ($tab === $activeTab) {
        $html .= ' class="is-active" aria-current="page"';
    }
    return $html . '>' . htmlspecialchars($label, ENT_QUOTES, 'UTF-8') . '</a></li>';
}

function videos_rankings_render_videos($items, $cache, $configuration, $language)
{
    $html = '<section class="videos-ranking-section"><h2>'
        . htmlspecialchars($language['top_videos'], ENT_QUOTES, 'UTF-8') . '</h2>';
    if (count($items) === 0) {
        return $html . '<p>' . htmlspecialchars($language['ranking_no_data'], ENT_QUOTES, 'UTF-8') . '</p></section>';
    }
    ...
    foreach ($items as $videoId => $item) {
        ...
        $html .= '<li class="videos-ranking-item"><a class="videos-ranking-thumbnail" href="'
            . htmlspecialchars($url, ENT_QUOTES, 'UTF-8') . '">';
        if (strpos($thumbnail, 'https://') === 0) {
            $html .= '<img loading="lazy" src="' . htmlspecialchars($thumbnail, ENT_QUOTES, 'UTF-8')
                . '" alt="' . htmlspecialchars(VIDEOS_thumbnailAlt($title, $channelTitle), ENT_QUOTES, 'UTF-8') . '">';
        }
        $html .= '</a><div class="videos-ranking-content"><h3><a href="'
            . htmlspecialchars($url, ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . '</a></h3>';
        if ($channelTitle !== '') {
            if (Videos_Validator::youtubeChannelId($channelId)) {
                $html .= '<p class="videos-ranking-channel"><a href="'
                    . htmlspecialchars(plugin_idtourl_videos('', 'channel:' . $channelId), ENT_QUOTES, 'UTF-8') . '">'
                    . htmlspecialchars($channelTitle, ENT_QUOTES, 'UTF-8') . '</a></p>';
            } else {
                $html .= '<p class="videos-ranking-channel">' . htmlspecialchars($channelTitle, ENT_QUOTES, 'UTF-8') . '</p>';
            }
        }
        $html .= videos_rankings_metrics($item, $language) . '</div></li>';
    }
...
{
    $html = '<section class="videos-ranking-section"><h2>'
        . htmlspecialchars($language['top_channels'], ENT_QUOTES, 'UTF-8') . '</h2>';
    if (count($items) === 0) {
        return $html . '<p>' . htmlspecialchars($language['channel_ranking_no_data'], ENT_QUOTES, 'UTF-8') . '</p></section>';
    }
    ...
    foreach ($items as $channelId => $item) {
        ...
        $html .= '</h3>' . videos_rankings_metrics($item, $language)
            . '<p class="videos-ranking-meta">' . htmlspecialchars($language['channel_videos'], ENT_QUOTES, 'UTF-8')
            . ' : ' . (isset($item['video_count']) ? (int) $item['video_count'] : 0)
            . '</p></div></li>';
    }
    return $html . '</ol></section>';
}
...
{
    $average = isset($item['rating_average']) ? (float) $item['rating_average'] : 0;
    $ratings = isset($item['rating_count']) ? (int) $item['rating_count'] : 0;
    $views = isset($item['view_count']) ? (int) $item['view_count'] : 0;
